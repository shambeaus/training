...
alldetail = sw1_show_cdp_neighbors_detail + r1_show_cdp_neighbors_detail

device_dic = {}

detail_split = alldetail.split('\n')

for line in (detail_split):
    if 'Device' in line:
        device = line
        formatdevice = device.split(':')
        deviceID = formatdevice[1]
        device_dic[deviceID] = {}
        continue

    elif 'IP address' in line:
        IP = line
        formatIP = IP.split(' address:')
        deviceIP = formatIP[1]
        device_dic[deviceID]['IP'] = deviceIP
        continue
    elif 'Capabilities' in line:
        cap = line
        if 'Router' not cap
            device_dic[deviceID]['Type'] = 'Switch'
        else:
            device_dic[deviceID]['Type'] = 'Router'
        continue
    elif 'Platform' in line:
        platform = line
        if 'Cisco' in platform:
            device_dic[deviceID]['Vendor'] = 'Cisco'
            continue
        else:
            device_dic[deviceID]['Vendor'] = 'not cisco device'
            continue
        continue
